async function filtroEstoque(event) {
    event.preventDefault();

    const nome = document.getElementById("pesquisarNome").value.trim().toLowerCase();
        ...
        filtrados.forEach(item => {
            const linha = `
                <tr>
                    ...
                    <td>${item.valor ? parseFloat(item.valor).toFixed(2) : ''}</td>
