...
public class CadastroProdutosDAO {
    ...
    public boolean salvar(CadastroProdutoModel produto) {
        // Corrigido: SQL com 10 colunas (adicionado status) e 10 parametros correspondentes
        ...
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            if (produto.getDataFabricacao() == null || produto.getDataFabricacao().isBlank()) {
                stmt.setDate(5, null);
            } else {
                stmt.setDate(5, java.sql.Date.valueOf(produto.getDataFabricacao()));
            }
            if (produto.getDataVencimento() == null || produto.getDataVencimento().isBlank()) {
                stmt.setDate(6, null);
            } else {
                stmt.setDate(6, java.sql.Date.valueOf(produto.getDataVencimento()));
            }
            stmt.setLong(7, produto.getQuantidade());
            stmt.setBigDecimal(8, new java.math.BigDecimal(produto.getValor()));
            stmt.setBigDecimal(9, new java.math.BigDecimal(produto.getTotal()));
    ...
    public List<CadastroProdutoModel> listar() {
        ...
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            while (rs.next()) {
                ...
                // Corrigido: typo "cadigo_barras" -> "codigo_barras"
